# -*- coding: utf-8 -*-
"""
Команда для аналізу справ банкрутства за допомогою Anthropic Claude API.
Обробляє справи з контролем швидкості та ліміту запитів.
"""

from django.core.management.base import BaseCommand
from django.db import connection
from bankruptcy.models import BankruptcyCase, TrackedCourtDecision, LLMAnalysisLog
from bankruptcy.anthropic_analyzer import get_anthropic_analyzer
from bankruptcy.trigger_words import has_both_triggers_in_same_sentence
import time

class Command(BaseCommand):
    help = "Аналізує справи банкрутства за допомогою Anthropic Claude API"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"🤖 АНАЛІЗ СПРАВ ANTHROPIC CLAUDE API")
        ...
        if not analyzer.test_connection():
            self.stdout.write(self.style.ERROR("❌ Не вдалося підключитися до Anthropic API"))
            self.stdout.write("Перевірте API ключ та інтернет з"єднання")
            return
        
        self.stdout.write(self.style.SUCCESS("✅ З"єднання з Anthropic API встановлено"))
        ...
        for case in cases_to_process:
            ...
            try:
                ...
                for i in range(0, len(trigger_resolutions), batch_size):
                    ...
                    self.stdout.write(f"   ⏳ Відправка запиту до Claude API...")
